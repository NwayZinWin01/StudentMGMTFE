<template>
  ...
  <div>
      <v-container v-show="isExactActive">
                        ...
                        <v-col cols="12" md="3">
                            <v-autocomplete v-model="model.course_id"  clearable label="Course Name" variant="outlined"
                                :items="courselist" item-title="title" item-value="id"></v-autocomplete>
                                    ...
                                    <v-col cols="12"  md="3">
                            <v-text-field v-model="model.attendence_date" label="Attendence Date" variant="outlined"
          ...
          <v-row>
              <v-col cols="12" md="2">
                  <router-link to="/attendence/attendenceentry">
                    ...
                    <template v-slot:item.action="{ item }">
                            <v-tooltip location="top">
                                <template v-slot:activator="{ props }">
                                    <router-link :to="{ name: 'attendenceentry', query: { id: item.id } }">
...
{ title: " Action", align: "center", key: "action", sortable: false },
  ...
  { title: "Attendence Date", align: "center", key: "attendence_date" },
...
const DeleteItem = (val) => {
    if (val) {
        attendenceService.Delete(selected_id.value).then((res) => {           
            if (!res.data.success) {
