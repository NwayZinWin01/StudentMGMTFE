        ...
        <v-sheet class="mx-auto" width="350">
            <v-form ref="form" class="mt-2">
                <div class="mt-3">
                    <v-row>
                        <v-col>
                            <h2 class="d-flex justify-center">Assignment Information entry</h2>
                        </v-col>
                    </v-row>
                    <v-row class="mr-2 ml-2 mt-2">
                        <v-col cols="12">
                            <v-text-field v-model="model.title" clearable label="Title" :rules="[v => !!v || 'Name is required']" required
                                variant="outlined"></v-text-field>
                        </v-col>
                    </v-row>
                    <v-row class="mr-2 ml-2 mt-2">
                        <v-col cols="12">
                            <v-autocomplete v-model="model.course_id" clearable label="Course Name" variant="outlined" :rules="[v => !!v || 'Name is required']" required
                                :items="courselist" item-tilte="name" item-value="id"></v-autocomplete>
                        </v-col>
                    </v-row>
                    <v-row class="mr-2 ml-2 mt-2">
                        <v-col cols="12">
                            <v-text-field v-model="model.description" clearable label="Description" :rules="[v => !!v || 'Description is required']" required
                                variant="outlined"></v-text-field>
                        </v-col>
                    </v-row>
                    <v-row class="mr-2 ml-2 mt-2">
                        <v-col cols="12">
                            <v-text-field v-model="model.due_date" label="Due Date" variant="outlined" :rules="[v => !!v || 'Date is required']" required
...
onMounted(() => {
    ...
    if (assignmentId) {
        console.log("Fetching course details for ID:", assignmentId);
...
function SaveOrUpdate() {
    ...
    AssignmentService.SaveOrUpdate(model.value)
        .then((res) => {
            if (res.data.success) {
                color.value = "success";
            } else {
